...
int pcd_request_b(u8 req_code, u8 AFI, u8 N, u8 *ATQB)
{
    ...
    {	
        ...
        g_fwi = (ATQB[11]>>4);
		g_pcd_module_info.uiFwi = g_fwi;//(ATS[2+(u8)ta] & 0xF0) >> 4;
		g_pcd_module_info.uiSfgi = ATQB[12] >> 4;
...
int MH1608_SendFrame(u8 *pid,int inlen,int *outlen)
{
    int XDATA status;
	...
	memcpy(mf_com_data.mf_data,pid,inlen);

	status = pcd_com_transceive(pi);
#if (NFC_DEBUG)
	printf("mf_com_data.mf_length = %d\n",mf_com_data.mf_length);
#endif

    if (status == MI_OK) 
    {	
	memcpy(pid, &mf_com_data.mf_data[0], mf_com_data.mf_length/8);
	*outlen = mf_com_data.mf_length/8;
    }
		else
		{
		    *outlen = 0;
		}
	//	printhex("pi.mf_data",pi->mf_data,pi->mf_length);
	    return status;
}

int MH1608_SendFrame2(u8 *pid,int inlen,int *outlen)
{
    int XDATA status;
...
#if (NFC_DEBUG)
	printf("SELECT IDCARD:\n");
#endif
	//pcd_set_tmo(5);
	
    mf_com_data.mf_command = PCD_TRANSCEIVE;
    mf_com_data.mf_length  =12;
    mf_com_data.mf_data[0] =0x1D; //ISO14443B_ANTICOLLISION;     	       // APf code
    mf_com_data.mf_data[1] =0x00;// AFI;                // 
    mf_com_data.mf_data[2] =0x00; //((req_code<<3)&0x08) | (N&0x07);  // PARAM
	mf_com_data.mf_data[3] =0x00;
	mf_com_data.mf_data[4] =0x00;
 	mf_com_data.mf_data[5] =0x00;
	mf_com_data.mf_data[6] =0x86;
	mf_com_data.mf_data[7] =0x0C;
	mf_com_data.mf_data[8] =0x00;
	mf_com_data.mf_data[9] =0x80;
	mf_com_data.mf_data[10] =0x80;
	mf_com_data.mf_data[11] =0x00;
...
int MH1608_ID_Command(unsigned char *data_in,uint16_t dataInLen,unsigned char *data_out,uint16_t *dataOutLen)
{
    int XDATA status;
    ...
    mf_com_data.mf_command = PCD_TRANSCEIVE;
    mf_com_data.mf_length  =dataInLen;
    memcpy(mf_com_data.mf_data,data_in,dataInLen);
    ...
    {	
        ...
        //pcd_set_tmo(ATQB[11]>>4); // set FWT 
        //lite_printf("length:%d\r\n",mf_com_data.mf_length);
		*dataOutLen = mf_com_data.mf_length/8;
		memcpy(data_out, &mf_com_data.mf_data[0], mf_com_data.mf_length/8);
    } 
    else
    {
        *dataOutLen = 0;

...
//-------------------------------------------
int PiccWupa(unsigned char *atqa)
{
     int status;
     status = pcd_request(0x52, g_tag_info.tag_type_bytes);
     if(status == MI_OK)
     {
         if(atqa)
         {
             atqa[0] = g_tag_info.tag_type_bytes[0];
             atqa[1] = g_tag_info.tag_type_bytes[1];
         }
     }
   //  lite_printf("PiccWupa:%d,%d\r\n",status,-1);
     return status;
}

int PiccWupb(unsigned char *atqb)
{
     int status;
     unsigned char ATQB[20];
     status = pcd_request_b(0x08, 0, 0, ATQB);
     if(status == MI_OK)
     {
         if(atqb)
         {
...
int com_reqb(u8 *pcmd)
{
	...
	while(cnt--)
	{
	    memset(gucATQB, 0, sizeof(gucATQB));
		status = pcd_request_b(req_code, 0, 0, ATQB);

		g_tag_info.uid_length = 4;
		memcpy(g_tag_info.serial_num,&ATQB[1],4);

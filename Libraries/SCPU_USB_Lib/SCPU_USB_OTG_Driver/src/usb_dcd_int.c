...
t_usb_context usb_context;

extern volatile unsigned int g_Usb_Recv_complete;
static unsigned int Usb_Send_Busy = 0;

uint32_t g_usbrecvcount = 0;

void Usb_Set_SendBusy(void)
{
    Usb_Send_Busy = 1;
}

void Usb_Clr_SendBusy(void)
{
     Usb_Send_Busy = 0;
}

int Usb_Get_SendBusy(void)
{
    return Usb_Send_Busy;
}
...
//	printf("rx_count.d16:%d, %d\r\n",rx_count.d16, pdev->dev.out_ep[epnum].maxpacket);	
                ...
                memcpy(Buf, &pdev->dev.out_ep[epnum].xfer_buff[0],64);
#if 1
			    if(usb_context.ofst_free + 64 <= USB_RECV_BUF_SIZE)
			    {
				    memcpy(&usb_context.buffer[usb_context.ofst_free], &pdev->dev.out_ep[epnum].xfer_buff[0], 64);
				    usb_context.ofst_free += 64;
				    usb_context.ofst_free = usb_context.ofst_free % USB_RECV_BUF_SIZE;
			    }
		           else
		           {
                                    memcpy(&usb_context.buffer[usb_context.ofst_free], &pdev->dev.out_ep[epnum].xfer_buff[0], USB_RECV_BUF_SIZE - usb_context.ofst_free);
                                    memcpy(&usb_context.buffer[0], 
						 	&pdev->dev.out_ep[epnum].xfer_buff[USB_RECV_BUF_SIZE - usb_context.ofst_free], 
						 	64 - (USB_RECV_BUF_SIZE - usb_context.ofst_free));
						 usb_context.ofst_free = 64 - (USB_RECV_BUF_SIZE - usb_context.ofst_free);
			    }
#else
                             if(Buf[0] < 64)
                             {
                                    for(i = 0; i < Buf[0]; i++)
                                    {
                                            usb_context.buffer[usb_context.ofst_free++] = Buf[1+i];
                                            usb_context.ofst_free  %= USB_RECV_BUF_SIZE;
                           //                 printf("%02x ",Buf[1+i]);
                                    }
                             }
#endif
                            ...
                            g_Usb_Recv_complete = 1;

                /*
                              {
                                    int i;
                                    printf("rx count:%d,%d\r\n",rx_count.d16,pdev->dev.out_ep[epnum].xfer_count);
                                    for(i = 0; i < 64;i++)
                                    {
                                         printf("%02x ",pdev->dev.out_ep[epnum].xfer_buff[i]); 
                                    }
                                    printf("\r\n");
                                }
                
                                  printf("rx cmplete:%d,%d\r\n",rx_count.d16,pdev->dev.out_ep[epnum].xfer_count);*/

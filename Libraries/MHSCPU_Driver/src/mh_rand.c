...
__STATIC_INLINE void mh_trand_init(void)
{
	...
	MH_TRNG->RNG_AMA &= ~TRNG_RNG_ANA_PD_TRNG0_Mask;
	
	MH_TRNG->RNG_AMA &= ~TRNG_RNG_ANA_LFSR_XOR_FB_Mask;	//默认LFSR反馈自身
}

__STATIC_INLINE void mh_trand_start(void)
{
	MH_TRNG->RNG_CSR &= ~TRNG_RNG_CSR_S128_TRNG0_Mask;
}
...
__STATIC_INLINE uint32_t mh_trand_get(uint32_t rand[4])
{
	 ...
	 */
	while (!(MH_TRNG->RNG_CSR & TRNG_RNG_CSR_S128_TRNG0_Mask));
	
	/*
	 *	check if the TRNG is attacked
	 */
	if (MH_TRNG->RNG_CSR & TRNG_RNG_CSR_ATTACK_TRNG0_Mask)
	{
		ret = 0;
		goto cleanup;
	}
